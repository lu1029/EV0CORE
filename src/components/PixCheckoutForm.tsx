...
import { Copy, Loader2, QrCode, Check } from "lucide-react";
...
export function PixCheckoutForm({ amountCents, description, defaultEmail }: PixCheckoutFormProps) {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("create-pix-qrcode", {
        body: {
          ...
          cpf: cpf.replace(/\D/g, ""),
          amount: amountCents,
          description: description || "Assinatura EvoCore Premium",
          expiresIn: 3600,
        },
      });
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
        <div className="bg-card border border-border/40 rounded-2xl p-5 flex flex-col items-center">
          ...
          <p className="mt-4 text-[13px] text-muted-foreground">
            Valor: <span className="text-foreground font-semibold">{formatBRL(result.amount)}</span>
          </p>
          {result.expiresAt && (
        ...
        <div className="mt-5">
          ...
          <div className="mt-2 flex gap-2">
            ...
            <Button onClick={copyCode} type="button" className="shrink-0">
        ...
        <button
          ...
          className="mt-4 w-full h-11 rounded-xl bg-secondary text-foreground text-[14px] font-medium active:opacity-70"
        >
          Gerar novo QR Code
        </button>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <Label htmlFor="pix-fullName">Nome completo</Label>
        <Input
          id="pix-fullName"
          required
          value={fullName}
      ...
      <div className="bg-card border border-border/40 rounded-xl p-4 mt-2">
        ...
        <p className="text-[22px] font-bold tracking-tight">{formatBRL(amountCents)}</p>
      </div>
